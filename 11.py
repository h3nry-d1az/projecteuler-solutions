...
def solution() -> None:
  ...
  for j, r in enumerate(grid):
    for i, n in enumerate(r):
      # Left and right
      if i >= 3: candidates.add(n * r[i-1] * r[i-2] * r[i-3])
      if i <= 16: candidates.add(n * r[i+1] * r[i+2] * r[i+3])
      # Up and down
      if j >= 3: candidates.add(n * grid[j-1][i] * grid[j-2][i] * grid[j-3][i])
      if j <= 16: candidates.add(n * grid[j+1][i] * grid[j+2][i] * grid[j+3][i])
      # Diagonally
      if i >= 3 and j >= 3: candidates.add(n * grid[j-1][i-1] * grid[j-2][i-2] * grid[j-3][i-3])
      if i <= 16 and j >= 3: candidates.add(n * grid[j-1][i+1] * grid[j-2][i+2] * grid[j-3][i+3])
      if i >= 3 and j <= 16: candidates.add(n * grid[j+1][i-1] * grid[j+2][i-2] * grid[j+3][i-3])
      if i <= 16 and j <= 16: candidates.add(n * grid[j+1][i+1] * grid[j+2][i+2] * grid[j+3][i+3])
  print(max(candidates)) # 70600674
